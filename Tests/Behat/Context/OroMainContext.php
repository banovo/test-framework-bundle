...
namespace TestFrameworkBundle\Tests\Behat\Context;
...
use TestFrameworkBundle\Behat\Context\AssertTrait;
use TestFrameworkBundle\Behat\Element\CollectionField;
use TestFrameworkBundle\Behat\Element\Form;
    ...
    KernelAwareContext
{
     ...
     *            | Date        | 2017-08-24      |
     *
     * @When I fill :formName form with:
     */
    public function iFillFormWith($formName = "OroForm", TableNode $table)
    {
        /** @var Form $form */
    ...
    /**
     * @Given I am on specifications page of :projectName project
     */
    public function iAmOnSpecificationsPageOfProject($projectName)
    {
        $repository = $this->getContainer()->get('doctrine')->getManager()->getRepository('AppBundle:CustomerProject');
        $all = $repository->findAll();

        var_dump($all);
        die;

        $entity = $repository->findOneBy(['name' => $projectName]);
        $uri = $this->getContainer()->get('router')->generate('app_customerproject_view', [
            'id' => $entity->getId()
        ]);
        $this->visit($uri);
    }
    ...
    /**
     * @When I click row number :rowNumber in table with :text
     */
    public function iClickRowNumber($rowNumber, $text)
    {
        $this->getSession()->evaluateScript('
            $(document).ready(function($){
                var el = $(\'table:contains("'.$text.'") tr:eq('.$rowNumber.')\')[0];
    ...
    {
        // todo: I changed from "OroForm" to "Login" in order to fix "log in as administrator"
        return $this->createElement('Login');
    }
